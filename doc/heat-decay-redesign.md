...
### Phase 4 -- exponential redistribution: no floor, no clamp, provable by construction (DONE)

Working through whether `pool_scale`'s long-run numerical stability is provable (not just "probably fine") surfaced two separate things originally scoped as a clamp-plus-migration plan below. That plan is superseded by a smaller fix that solves both at once, with no tunable constant.

#### The bug this replaces: `f` was not guaranteed to stay inside `(0, 1)`

`f = 1 - delta / (T - x_eff)` is a straight line in `delta`. The old code only guarded `denominator <= 0` (falling back to `f = 1.0`, no shrink at all); it did not guard `0 < denominator < delta`. In a small pool where one document already holds nearly all the heat, `delta` (up to `0.5`, at tier 3) can exceed what's left in "the rest of the pool," driving `f` to zero or negative -- a real, reachable gap in the shipped formula, not a theoretical one.

The first fix considered was a clamp: cap `f` at a hand-picked floor `F_MIN`, chosen by simulating 20,000 events against a 1,000-document Zipf-skewed pool and checking the floor never engaged. That works, but the guarantee it provides is empirical ("didn't trigger in this simulation"), not structural, and it needed re-deriving `F_MIN` for a different traffic pattern. **Rejected in favor of a redistribution rule that's bounded by construction, for any input, not just the ones a simulation happened to try.**

#### The fix: make the redistribution multiplicative, not linear

```
f = exp(-delta / denominator)
extracted = denominator * (1 - f)     -- what the rest of the pool actually gives up
X_new = x_eff + extracted             -- X receives exactly what was extracted, never the raw nominal delta
```

Three things follow from this algebraically, none of them asserted:

- **`f` is in `(0, 1)` for *any* `delta >= 0` and any `denominator > 0`, with no floor.** `exp` of a non-positive real number is always strictly positive and always `<= 1`. There is no `delta` that breaks this -- not "very unlikely to," structurally can't.
- **Conservation is still exact**, by the same substitution as before: `X_new + denominator * f = x_eff + extracted + denominator * f = x_eff + denominator * (1 - f) + denominator * f = x_eff + denominator = T`.
- **It reduces to today's behavior for realistic traffic.** For the typical case (`delta/denominator ~= 0.00025`, see below), `1 - e^{-x} = x - x^2/2 + ...`, so `extracted ~= delta` to about 8 significant figures -- indistinguishable from the old linear formula for ordinary reinforcement. It only diverges once the pool is genuinely heat-concentrated, and diverges gracefully (X asymptotically receives less as the pool empties) rather than hitting the old clamp's discontinuous derivative at the `F_MIN` boundary. Concretely: the clamp's floor (`0.05`) is a value this curve reaches on its own at `delta/denominator = 3` (`e^-3 ~= 0.0498`), and keeps easing down after that instead of getting stuck.

The one case this doesn't cover is `denominator <= 0` itself (a single-document pool, or a badly drifted total) -- there's no "rest of the pool" to draw from regardless of which curve governs everyone else, so that stays its own branch: `extracted = 0`, no shrink applied, matching "a single-document pool must hold `x_eff` exactly at `BASE_HEAT` forever."

#### Where this lands relative to link-strength

`knowledge.spread_activation` calls `reinforce_pool_heat` once per linked neighbor, with each neighbor's `delta` sized by `doc/link-strength-redesign.md`'s weighting (`document.weighted_spreading_delta`, capped at `0.35 * base_delta`, provably bounded on its own terms). Link-strength doesn't need this fix -- its own delta can never itself exceed the direct-hit ceiling -- but it does make deltas close to that ceiling reachable in more real situations than the old flat `/fan_count` split did (a hub with one strongly-reinforced edge among several weak ones now concentrates most of its spread onto that one neighbor, rather than diluting it evenly). Both the direct-hit reinforcement and every spread-activation neighbor bump land in the same `reinforce_pool_heat`/`pool_scale` primitive, so the guarantee only has to hold at that one shared point -- which is exactly what this fix does, for both callers, unconditionally.

#### The representation change: only the shared `pool_scale` needs log-space

`pool_scale` decays toward zero under sustained reinforcement by construction (every event's `f < 1`, and nothing routinely pushes it back up). That's "cold sinks" working as designed, not a bug -- but it means the *shared* value is on an unbounded one-directional multiplicative decline for as long as the system sees net reinforcement, which is normal operation. Repeated multiplication by factors just under 1 is exactly the pattern that erodes floating-point precision: for a realistic pool (`T ~= 1000`, typical `delta ~= 0.25`), each event shrinks `pool_scale` by roughly `delta/T ~= 0.00025` in log terms, and double precision starts to degrade once accumulated `|ln(pool_scale)|` reaches `~708` -- about `2.8 million` events, not an unreachable number for a multi-year system.

**Only `knowledge_pool_state.pool_scale` moves to log-space** (`log_pool_scale`, additive updates: `log_pool_scale = log_pool_scale + ln(f)`, still a single relative atomic in-place update, `+` instead of `*`). Each document's own `scale_at_write` **stays linear, unchanged** -- it's a one-shot snapshot taken when that row was last written, never itself repeatedly multiplied, so it doesn't suffer the drift the shared value does. The shared log value is exponentiated back to an ordinary linear scalar (`pool_scale = exp(log_pool_scale)`) once per event, in application code, and that fresh linear value is what every per-row computation (`pool_effective_heat`, the per-row `raw_heat` update) already uses -- `document.pool_effective_heat`'s own signature and arithmetic are completely unchanged. This also sidesteps a real portability trap the original three-column plan below didn't: every per-row SQL `UPDATE` stays pure arithmetic (`*`, `/`, `+`), with no `EXP()`/`LN()` SQL function required from either SQLite or MariaDB -- the only transcendental calls are `math.exp`/`math.log` in Lua, confirmed present in this Luam build (`lmathlib.c`, stock Lua 5.1's `math` library, no new dependency).

**Migration:** `log_pool_scale` is added as its own column (`ALTER TABLE knowledge_pool_state ADD COLUMN log_pool_scale REAL NOT NULL DEFAULT 0.0`, and included directly in `KNOWLEDGE_POOL_STATE_SCHEMA` for fresh installs). For an install that already has a non-default `pool_scale` from real traffic, the same migration step seeds `log_pool_scale = ln(pool_scale)` once, from the existing linear value -- a genuine migration, not a blind reset to `0.0`. `pool_scale` itself is left in the schema, simply no longer written or read -- the same fast-rollback shape Phase 3 used for the old `heat`/`last_retrieved_at` columns, not a bare rename with no way back.

**Deliberate scope decision, deviating from this phase's original three-step plan:** the original plan called for dark-launching the log columns for a full release before cutting reads over, mirroring Phase 2/3's own soak period. That precedent was earned by Phase 2/3 changing the *relevance model itself* (wall-clock decay to a conserved pool) -- a real behavior change worth validating against live traffic before it drove ranking. Phase 4 changes neither the model nor, for realistic traffic, its output (the ~1e-8 relative deviation above) -- it's a bug fix plus a numerically exact reparameterization of one already-validated quantity. Soaking that in parallel for a release would be re-deriving a validation Phase 2/3 already did, not earning a new one. Cut over directly; the old `pool_scale` column is the rollback path if this analysis turns out to be wrong.

#### A second gap this surfaced: `document_count` could drift for any document not created via `document.create_page`

Removing the old code's silent heat-manufacturing in the `denominator <= 0` branch exposed a real, separate, pre-existing gap: only `document.create_page` (the web save route, the agent's `document.create` tool) ever called `register_pool_document`. Every generic, entity-type-agnostic creation path -- `entity create`/`entity create-json` (CLI), the bulk-import and v1 API routes in `cgi.lua`, the agent's generic `entity.create` tool, extension-manifest creation -- went through `entity.create`/`entity.create_batch` directly and silently skipped it, the same category of gap `document.do_document`'s own comment already names for `sync_links`/`reindex_embedding` (with `resync-links`/`reindex-embeddings` as their backfill commands). Pool registration had no equivalent backfill, and no real-time guarantee either.

**Fixed at the single choke point, not by hooking every call site.** `entity.create` itself (`src/entity.lua`) now calls a new `document.on_entity_created(db_path, entity_type, entity_id)` hook unconditionally at the end of every successful create -- mirroring `on_entity_archived`/`on_entity_unarchived`'s own type-filtering shape (`if entity_type != "document" then return end`), via the same lazy, function-scoped `require("document")` those already use (safe for the same reason: this only ever runs during real request/CLI handling, long after every module has finished loading -- never at module-load time, which is what would actually be circular). Since `entity.create_batch` itself calls `entity.create` per row, and every generic creation path already funnels through one of those two functions, this covers all of them at once -- structurally, not by enumerating callers, so a new caller added later can't reintroduce the gap by forgetting the hook. `document.create_page` no longer calls `register_pool_document` itself (would now double-register); `entity.create`'s hook does it for that path too.

**Backfill, for whatever this still can't reach:** a raw SQL insert, a restored backup predating this hook, anything that never goes through `entity.create` at all. `document.resync_pool_count` (`daat repair pool-count` on the CLI) recomputes `document_count` from a live `COUNT(*)` over active documents -- the same one-time scan `ensure_pool_state`'s own initial seed already uses, just callable again on demand. It can't retroactively recover an individual mis-created document's own `raw_heat`/`scale_at_write` (there's no way to know what `pool_scale` was at whatever moment it should have been registered), but it does realign the aggregate invariant for every reinforcement from that point on, which is what the conservation guarantee actually depends on going forward.

## Critical files
- `/root/projects/daat/src/document.lua` -- `heat`/`retrieval_count`/`last_retrieved_at` columns (heat to become `raw_heat`/`scale_at_write`), `reinforcement_delta`, `effective_heat`, `search_score`, the pool primitives (`ensure_pool_state`, `register_pool_document`, `resync_pool_count`, `reinforce_pool_heat`, `return_pool_heat`, `on_entity_created`/`on_entity_archived`/`on_entity_unarchived`), and `do_document`'s `resync-pool` CLI action
...
- `/root/projects/daat/src/entity.lua` -- `entity.create`'s own single choke point, calling `document.on_entity_created` at the end of every successful create (Phase 4); every `entity.archive`/`entity.unarchive` call site (`entity.lua`'s own CLI dispatcher, plus `cgi.lua`/`agent.lua`) still calls `document.on_entity_archived`/`on_entity_unarchived` individually, the pattern Phase 4's create-side hook deliberately moved away from. All of these use a lazy, function-scoped `require("document")` rather than a module-top-level one, since `entity.lua` must never require `document.lua` (the reverse already holds)
- `/root/projects/daat/tst/unit/document_pool.lua` -- the conservation-invariant tests, including the Phase 4 bug-scenario regression test
- `/root/projects/daat/tst/integration/knowledge.bats` -- the two tests whose literal `raw_heat`/tier expectations changed under exponential redistribution (a single-document pool has no room to reinforce; a single retrieval can no longer be counted on to cross the heat threshold, only `retrieval_count >= 2` reliably does)
