...
- **Nav integration is nearly free.** `html.page_shell`'s `nav_items` (`src/html.lua:1310`) is already exactly the shape a plugin nav entry needs: a plain ordered array of `{key, href, label, icon}`, built once per request from a few `if` checks. Appending approved-extension-declared entries to that same array, in the same loop that already renders it, is a small addition. This overlapped directly with brex 683042859 (configurable nav order/visibility) -- both wanted "the nav rail is an ordered, filterable list from more than one source." That task shipped the shared mechanism: `platform.lua`'s own `nav_order`/`nav_hidden` (`html.lua`'s `apply_nav_order`/`apply_nav_hidden`) are applied as a deployment-wide overlay *after* both the hardcoded built-ins and any approved-extension entries are already in the list -- an extension's own `key` (`"ext:<name>"`) can be reordered or hidden by a deployment the exact same way a built-in item can, with no special-casing needed for which source it came from.
...
- **Dispatch/storage details** for both surfaces (where plugin-declared routes get matched in `cgi.lua`'s dispatch chain, how a tool-call's `ctx` differs from an entity-hook's `ctx`, how approval scopes to "this route" / "this tool" versus the whole extension) -- real implementation work, not yet designed here. *(Update: one piece of this landed with the first real `capabilities.ui` extension, `experiment_view` (brex 462301011) -- `GET /ext/<name>` (`cgi.lua`) now calls `hooks.render(ctx, params)`, `params` being the exact same request-query-string table every other route already reads, not a plugin-specific parameter scheme. `POST /ext/<name>/action` already passed a generic `args` body the same way; this just makes the GET path consistent with it.)*
