...
- Node color: tier. Worth flagging directly rather than assuming: `/knowledge`'s own tier tiles (`html.render_knowledge_pool`) have no distinct per-tier color today -- every tile shares one neutral style, differentiated only by label/caption text. A 4-color tier palette would be new here, not reused, and should stay consistent with whatever this repo's own theme-color conventions are (`theme.lua`'s `colors` fields) rather than inventing an unrelated one.
