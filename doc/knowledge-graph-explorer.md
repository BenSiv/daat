...
Follows `/knowledge`'s own established pattern exactly (`src/cgi.lua:816-853`): a new route, `/knowledge-graph`, wrapped in the same `html.page_shell`, linked from `/knowledge`'s own landing page as another drill-down view alongside `/knowledge-documents` and `/knowledge-reviewed` -- not a new top-level nav-rail entry (though the Knowledge Pool as a whole now has one, see below), the same "one link away, not a dedicated icon" reasoning `doc/architecture.md`'s "Chat" section already applies to `/chat`.

**Resolved (brex 492390825):** the open gating question below was answered by moving the whole `/knowledge*` family, including this route, to baseline capability instead of Setup/Admin -- it's read-only browsing, not an admin action, and the Knowledge Pool now has its own top-level nav icon rather than living under `/system`. Left verbatim below for the historical record of how that call got made.

**Open question, not resolved here [now resolved above]:** `/knowledge`'s existing pages are Setup/Admin-gated because they're pool-health/analytics views. A graph *explorer* is arguably a different kind of thing -- closer to a navigation aid any user browsing their own accessible documents would want, the way Obsidian's graph view isn't an admin feature in Obsidian itself. Keeping the same gate is the conservative default (stated here, not silently assumed); loosening it to any authenticated user is a real, separate product decision this doc isn't making.
...
Resolved: route/gating follows `/knowledge`'s own precedent, data via a new JSON endpoint (not server-rendered), vanilla-JS force layout on canvas (no new dependency), node size = heat / edge weight = strength / node color = tier. Open at the time: whether the Setup/Admin gate is actually right for an explorer rather than an analytics page (see above) -- since resolved, moved to baseline (brex 492390825).
...
**Collision avoidance and a live-tunable Forces panel.** Two real usability problems showed up on real pools: large, high-heat (large-radius) nodes could settle overlapping each other, since the repulsion force above is pure inverse-square and radius-blind -- two big nodes and two tiny ones get pushed apart exactly as hard. And small, loosely-connected clusters tended to drift into the same central "pool" as everything else, since the single global `CENTER_PULL` pulls every node toward one point regardless of which component it belongs to. Fixed two ways, both staying inside the existing hand-rolled, same-file, no-new-dependency approach:
- A hard collision floor added directly into the existing repulsion loop (`COLLISION_PADDING`/`COLLISION_STRENGTH`): when two nodes' circles (by `nodeRadius(heat)`) would overlap, an extra separation force pushes them apart until they don't, on top of whatever the base charge repulsion already produced. Always on, not user-tunable -- it's a correctness floor, not a taste preference.
- A collapsible "Forces" panel (Obsidian's own graph settings have the identical four sliders) exposing `REPULSION`, `SPRING` (link force), `SPRING_LENGTH` (link distance), and `CENTER_PULL` (center force) as live range inputs -- each `input` event reassigns the matching var and calls the existing `wakeSimulation()`, so a drag is visible immediately, same as dragging a node. This is deliberately manual, not automatic connected-component clustering: the user spreads clusters apart themselves by raising Repel/lowering Center, exactly how Obsidian's own graph view works (it doesn't auto-cluster either). Slider values persist per-browser in `localStorage` (`platform-kg-physics-v1`), and the panel's own open/closed state in `platform-kg-controls-open`, both following the same try/catch-wrapped, per-viewer-only convention as the layout cache above and the chat widget's `OPEN_KEY`.

**Verified:** manual check (same limitation as the rest of Phase 3 -- bats can't drive canvas/slider interaction) plus one new automated test asserting the Forces panel's markup renders (`tst/integration/knowledge.bats`).
