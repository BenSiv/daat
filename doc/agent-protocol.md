...
| `type` | shape | meaning |
|---|---|---|
| `text` | `{type="text", text=..., thinking_signature=...}` | ordinary reply text |
| `toolCall` | `{type="toolCall", id=..., name=..., arguments=..., thinking_signature=...}` | the model wants to call `name` (a dotted `"tool.method"` string matching an `AGENT_TOOLS` entry) with `arguments` |
| `thinking` | `{type="thinking", thinking=..., thinking_signature=...}` | reasoning content, shown inline in `/chat` and fed into the Knowledge Pool reasoning-document pipeline |

`thinking_signature` (on any of the three block types above) is optional, provider-specific continuity data round-tripped opaquely -- captured from whichever block a provider's response attached it to, and sent back unchanged on that same block in a later request, never inspected or generated by `agent.lua` itself. Some providers make this genuinely load-bearing rather than a nicety: `src/provider/agent_vertex.lua`'s own header documents a real, confirmed-live case (Gemini 3.x rejects a `toolCall` follow-up that omits one with a 400) alongside the merely-recommended case (a plain `text`/`thinking` block's own signature).
