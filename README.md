...
daat user invite <login> <email> [cap]        # needs mail: creates the login and emails a setup link
daat user email <login> <email>               # "" clears it; with mail, emails a setup link to verify it
...
**Forgot password.** When `platform.lua` sets `mail_provider`, `mail_from` and `public_url`, `/login` links to `/forgot-password`: a user enters their login or email, and if that matches an active account with a verified email, a one-time link to `/reset-password` is emailed to it (valid 1 hour, at most one new link per account every 5 minutes). Admin (`a`) accounts are excluded -- another admin resets them on `/admin-users` -- so a compromised mailbox can't take over the platform. The token travels in the link's `#fragment`, which browsers never send to the server, so it never appears in web server or load balancer logs. The response is the same whether or not anything matched. **Account setup.** With mail configured, an admin creates an account on `/admin-users` (or with `daat user invite`) from a login and an email, with no password: the account is emailed a setup link (valid 7 days) through which the person chooses their password, and using it is what verifies the email. Reset links only ever go to a verified address, so a mistyped one never receives them. Emails are set only by an admin, on `/admin-users` or with `daat user email`. A new or changed address starts unverified and gets a setup link, and saving an unverified address again resends it. `/account` shows the email but doesn't let the user change it, because otherwise anyone holding an open session could redirect the account's recovery. Without mail, `daat user add` and the `/admin-users` form take a password as before. The only real backend is `mail_provider = "smtp"`: `smtp_url` (e.g. `"smtp://smtp.gmail.com:587"`; TLS is always required), optional `smtp_user`, and the password in the `PLATFORM_SMTP_PASSWORD` environment variable. Without all three settings the flow is off and both routes 404.
...
**Co-retrieval linking, judged, not assumed.** Two documents that keep turning up together in the same searches get evaluated by the agent -- a genuine relationship, or coincidental overlap? -- and, once judged real, the agent writes the connection down the way a person would: a short note in the Knowledge Pool that links both and says why. A linked neighbor's retrieval gives the other document a smaller heat boost too (spreading activation, diluted by how many other links it already has), which is how cross-references form without anyone tagging anything by hand.
