...
agent_tools.AGENT_TOOLS = {
    ...
    knowledge = {
        ...
        distill = {
            destructive = true,
            description = "Write a new, concise, single-idea document distilled from a source you've actually read (e.g. via entity.get). Not a raw copy -- extract the one core idea in your own words, stated as a real hypothesis, finding, or definition that asserts something -- never as a subject/topic label that just names an area (e.g. \"Cryopreservation vs. Genetic Fidelity Thresholds\" names a topic without claiming anything about it; state the actual finding instead). Before writing, check whether this same idea already has a distilled record (e.g. document.search on its topic) -- don't create a near-duplicate for an idea that's already captured. Only do this for a source that's genuinely \"developed\" (long/multi-section) -- an already-\"atomic\"/\"thin\"/\"simple\" source has nothing worth extracting.",
            parameters = {
                type = "object",
                properties = {
                    title = {type = "string"},
...
Use knowledge.list to see current pool documents: id, tier, content shape (developed / atomic / thin / simple), effective heat, retrieval count. For a document flagged "developed" (long, multi-section, covers more than one real idea), read its full content with entity.get (entity_type=document) and write ONE genuinely atomic, single-idea document distilled from it with knowledge.distill -- concise, self-contained, in your own words, not a verbatim copy of the source, and stated as a fact or concept, not as a reminder, to-do, or assignment of who must do what. A source document's most prominent content is sometimes an action item or reminder rather than a stated idea -- that is not itself something to distill; only write a note if there's a genuine underlying fact or concept beneath it worth stating in its own right. Phrase it as a real claim -- a hypothesis, finding, or definition -- never as a subject/topic label that just names an area without asserting anything about it. Before distilling, check knowledge.list/document.search for whether a similar idea already has a distilled record -- don't create a near-duplicate for an idea that's already captured. Do not distill from a document that's already "atomic", "thin", or "simple" -- there's nothing worth extracting that isn't already there as-is.
