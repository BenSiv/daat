-- tst/unit/agent_claude.lua
-- Unit tests for src/provider/agent_claude.lua's translation functions:
...
agent_claude = require("provider.agent_claude")
json = require("dkjson")
...
function test_tools_from_canonical_passes_lowercase_schema_through()
    ...
    out = agent_claude.claude_tools_from_canonical(tools)
...
function test_messages_from_canonical_user_and_assistant()
    ...
    out = agent_claude.claude_messages_from_canonical(messages)
...
function test_blocks_from_content_all_three_types()
    ...
    blocks = agent_claude.claude_blocks_from_content(content)
...
function test_stop_reason_mapping()
    ...
    check(agent_claude.claude_stop_reason_to_canonical("end_turn") == "stop", "expected 'end_turn' -> 'stop'")
    check(agent_claude.claude_stop_reason_to_canonical("tool_use") == "toolUse", "expected 'tool_use' -> 'toolUse'")
    check(agent_claude.claude_stop_reason_to_canonical("max_tokens") == "length", "expected 'max_tokens' -> 'length'")
    ...
    check(agent_claude.claude_stop_reason_to_canonical("pause_turn") == "error", "expected unreachable-in-practice 'pause_turn' -> 'error'")
    check(agent_claude.claude_stop_reason_to_canonical("stop_sequence") == "error", "expected 'stop_sequence' -> 'error'")
    check(agent_claude.claude_stop_reason_to_canonical(nil) == "error", "expected nil -> 'error'")
end
...
print("All agent_claude.lua tests passed")
